...
public class TitlePlayerModelMover : MonoBehaviour
{
    ...
    private void Start()
    {
        var model = Instantiate(resourceContainer.playerModels[dataManager.playerSelect], transform, true);
        model.GetComponent<MeshRenderer>().sharedMaterial = resourceContainer.playerColors[dataManager.playerColorSelect];
